...
public class MovieService extends MovieServiceBase {
	...
	public Document findMovieByTitle(String title) {
		...
		return movies.find(query).first();
	}
	...
	public FindIterable<Document> getBestMovies(int minVotes, double minRating, int limit) {
		//TODO : implement
		return movies.find(and(gte("rating", minRating), gte("votes", minVotes)))
				.sort(new BasicDBObject("rating", 1)).limit(limit);
	}
	...
	public FindIterable<Document> getByGenre(String genreList, int limit) {
		...
		//TODO : implement
		return movies.find(and(all("genre", genres))).limit(limit);
	}
	...
	public FindIterable<Document> searchByPrefix(String titlePrefix, int limit) {
		//TODO : implement
		return movies.find(new BasicDBObject("title", Pattern.compile("^"+titlePrefix+".*"))).limit(limit);
	}
	...
	public FindIterable getTweetedMovies() {
		//TODO : implement
		return movies.find(new BasicDBObject("tweets", new BasicDBObject("$exists", true)));
	}
	...
	public void saveMovieComment(String id, String comment) {
		// TODO implement
		movies.updateOne(new BasicDBObject("_id", id), new BasicDBObject("$set", new BasicDBObject("comment", comment)));
	}
	...
	public FindIterable getByTweetsKeywordRegex(String keyword, int limit) {
		//TODO : implement
		return movies.find(new BasicDBObject("tweets.text",
				new BasicDBObject("$regex", Pattern.compile(".*" + keyword + ".*")))).limit(limit);
	}
	...
	public FindIterable<Document> searchTweets(String query) {
		...
		// TODO: implement
		return tweets.find(new Document("$text", new Document("$search", query)));
	}
	...
	public FindIterable<Document> getGeotaggedTweets(int limit) {
		//TODO : implement
		return tweets.find(new BasicDBObject("coordinates", new BasicDBObject("$exists", true)))
				.sort(new BasicDBObject("_id", -1)).limit(limit);
	}
